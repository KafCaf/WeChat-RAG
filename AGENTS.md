...
## 项目
RAG 智能问答系统，面向政策文档。三项目（国际杰青计划 + 科技部培训班 + 选房通知），57 chunks。双前端（React + 微信小程序）。
...
- 小程序: miniprogram/ — AppID wx3abc7e5d77ee3124，备案中
...
- 域名: rag-ai.top（ICP 备案已提交，等待审核）
- 外网: https://rag-ai.top/（安全组 22/80/443），6006 仅内网
...
| 项目 | 文档 | Chunks |
|---|---|---|
| 国际杰青计划 | 管理办法修改草案（17→25 年对照）.docx | ~45 |
...
| /projects | GET | 项目列表 |
| /files | GET | 文档列表 |
| /files | DELETE | 删除文档（不需登录） |
| /projects/{name} | DELETE | 删除项目（不需登录） |
| /register | POST | 注册 |
| /login | POST | 登录 |
...
- 文件上传: 支持 .docx/.pdf/.txt/.xlsx，MD5 查重
